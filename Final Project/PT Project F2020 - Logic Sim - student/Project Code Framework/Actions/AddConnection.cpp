...
void AddConnection::Execute()
{
	...
	if (component != NULL)
	{
		...
		if (led != NULL)
		{
			...
			M_GfxInfo.y2 = m_GfxInfo1.y2 - UI.LED0_Height / 2;
		}
	}
	...
	if (outpin != NULL && inpin != NULL) {
		Gate* gate = dynamic_cast<Gate*>(pManager->load_connection(id2));
		if (gate != NULL)
		{
			if (gate->Isconnectto(n)) {
